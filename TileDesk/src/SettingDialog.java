 ...
 * SettingDialog
 * 
 * UI design matching ProductMenuDialog with two options:
 * 1. Change Password - Shows content in the right panel
 * 2. System Setting - Opens as dialog
 */
public class SettingDialog extends JFrame {
    ...
    public SettingDialog() {
        ...
        JLabel subtitle = new JLabel("Settings Management");
    ...
    private JPanel createTopBar() {
        ...
        JLabel title = new JLabel(" Settings Dashboard");
    ...
    SwingUtilities.invokeLater(() -> {
        ...
        closeBtn.addActionListener(e -> {
            int opt = JOptionPane.showConfirmDialog(this, 
                "Close Settings Dashboard?", "Confirm", JOptionPane.YES_NO_OPTION);
    ...
    private JPanel createMenuGrid(String[] items) {
        ...
        JPanel grid = new JPanel(new GridLayout(items.length, 1, 0, 
            Math.max(12, Math.round(12 * scaleFactor()))));
    ...
    if ("Change Password".equalsIgnoreCase(text)) {
    ...
    if ("System Setting".equalsIgnoreCase(text)) {
    addWelcomeContent();          // reset center heading
    openSystemSettingDialog();    // existing logic
    return;
}
    ...
    // Remove anything currently shown in right panel
    contentPanel.removeAll();
    contentPanel.revalidate();
    contentPanel.repaint();

    // Get bounds of the RIGHT white panel
    Rectangle rightPanelBounds = contentPanel.getBounds();
    Point rightPanelOnScreen = contentPanel.getLocationOnScreen();

    int dialogWidth = 700;  // Updated to match SetPasswordDialog size
    int dialogHeight = 550; // Updated to match SetPasswordDialog size

    int x = rightPanelOnScreen.x + (rightPanelBounds.width - dialogWidth) / 2;
    int y = rightPanelOnScreen.y + (rightPanelBounds.height - dialogHeight) / 2;

    // Open your existing dialog - FIXED CONSTRUCTOR CALL
    SetPasswordDialog dlg = new SetPasswordDialog(this, contentPanel); // Fixed line
    dlg.setSize(dialogWidth, dialogHeight);
    dlg.setLocation(x, y);
    dlg.setVisible(true);

    // After dialog closes → return heading
    addWelcomeContent();
}
    
    private void openSystemSettingDialog() {
    // Remove anything currently shown in right panel
    contentPanel.removeAll();
    contentPanel.revalidate();
    contentPanel.repaint();
    ...
    int dialogWidth = 750;  // Matches SystemSettingDialog size
    int dialogHeight = 650; // Matches SystemSettingDialog size

    int x = rightPanelOnScreen.x + (rightPanelBounds.width - dialogWidth) / 2;
    int y = rightPanelOnScreen.y + (rightPanelBounds.height - dialogHeight) / 2;

    // Open SystemSettingDialog
    SystemSettingDialog dlg = new SystemSettingDialog(this, contentPanel);
    dlg.setSize(dialogWidth, dialogHeight);
    dlg.setLocation(x, y);
    dlg.setVisible(true);
    ...
    private MenuBox findSelectedBox() {
        ...
        for (MenuBox mb : menuBoxes) 
            if (mb != null) return mb; // fallback
        return null;
    }
    
    private void addWelcomeContent() {
        contentPanel.removeAll();
        JLabel lbl = new JLabel("Welcome to Settings Dashboard");
    ...
    private void showCenteredTitle(String titleText) {
    contentPanel.removeAll();
    ...
    // Dummy AdminDashboardDialog class for compilation
