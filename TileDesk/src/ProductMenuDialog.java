 ...
 * ProductMenuDialog
 *
 * UI-only improvements (logic unchanged):
 * - Starts maximized to cover the whole screen.
 ...
 * - A single moving accent indicator animates to hovered/selected menu boxes (mirrors Admin UI).
 * - Opening Add/Update/Delete/View dialogs remains modal and centered over the right content area.
 *
 * Drop this file into the same package as your AdminDashboardDialog. Placeholder modal dialogs
 * for Add/Update/Delete/View are included as inner classes so this file compiles standalone.
 */
public class ProductMenuDialog extends JFrame {
    ...
    public ProductMenuDialog() {
        super("Product Dashboard");
        setUndecorated(true);
        ...
        JLabel subtitle = new JLabel("Product Management");
    ...
    private JPanel createTopBar() {
        ...
        JLabel title = new JLabel(" Product Dashboard");
        ...
        back.addActionListener(e -> {
            ...
            SwingUtilities.invokeLater(() -> {
                try {
                    // re-open admin dashboard (original behavior)
        ...
        closeBtn.addActionListener(e -> {
            int opt = JOptionPane.showConfirmDialog(this, "Close Product Dashboard?", "Confirm", JOptionPane.YES_NO_OPTION);
    ...
    private class MenuBox extends JPanel {
        ...
        private void onClick(String text) {
            // keep logic same as original: open modal dialogs
            if ("Add Product".equalsIgnoreCase(text)) {
    SwingUtilities.invokeLater(() -> {
        AddProductDialog dlg =
            new AddProductDialog(ProductMenuDialog.this, contentPanel);
        dlg.setVisible(true);
    });
    return;
}
            ...
            if ("Delete Product".equalsIgnoreCase(text)) {
                SwingUtilities.invokeLater(() -> {
                    // OPEN REAL DeleteProductDialog (placeholder in this file)
                    DeleteProductDialog dlg = new DeleteProductDialog(ProductMenuDialog.this, contentPanel); //error 
                    dlg.setVisible(true);
                });
                return;
            }

           if ("Update Product".equalsIgnoreCase(text)) {
    ...
    SwingUtilities.invokeLater(new Runnable() {
        ...
        public void run() {
            try {
                ...
                UpdateProductDialog dlg = new UpdateProductDialog(ProductMenuDialog.this, contentPanel);
                dlg.setVisible(true);
                
                // After dialog closes, update UI if needed
                SwingUtilities.invokeLater(() -> {
                ...
                e.printStackTrace();
                JOptionPane.showMessageDialog(ProductMenuDialog.this,
                    "Error opening Update Product Dialog:\n" + e.getMessage(),
                    "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    });
    return;
            ...
            else if ("View Products".equalsIgnoreCase(text)) {
                // Open the real ViewProductDialog (placeholder in this file)
                SwingUtilities.invokeLater(() -> {
                    ViewProductDialog dlg = new ViewProductDialog(ProductMenuDialog.this);
                    dlg.setVisible(true);
                });
                return;
            }
    ...
    private void addWelcomeContent() {
        ...
        JLabel lbl = new JLabel("Welcome to Product Dashboard");
    ...
    private void rescaleUi() {
        ...
        // Update topbar font (if exists)
        // Repaint left panel to redraw indicator with new sizes
    ...
    // These are minimal modal dialogs that center over the contentPanel area and block the owner
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ProductMenuDialog dlg = new ProductMenuDialog();
            dlg.setVisible(true);
        });
    }
}